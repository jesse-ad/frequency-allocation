...
public class NetworkVisualiser {
    ...
    public static void createImage(
        ...
        g.drawString("Cellular Frequency Allocation Network",40,45);

        g.setFont(new Font("Arial", Font.PLAIN, 14));

        g.drawString("Edges represent cells within the interference threshold",40,70);
        ...
        for (Cell cell : cells) {
            ...
            for (Cell neighbour : neighbours) {
                ...
                if (cell.id.compareTo(neighbour.id) < 0) {
                    ...
                    int x1 = scaleX(cell.easting,minEasting,maxEasting);

                    int y1 = scaleY(cell.northing,minNorthing,maxNorthing);

                    int x2 = scaleX(neighbour.easting,minEasting,maxEasting);

                    int y2 = scaleY(neighbour.northing,minNorthing,maxNorthing);
        ...
        for (Cell cell : cells) {
            ...
            int x = scaleX(cell.easting,minEasting,maxEasting);

            int y = scaleY(cell.northing,minNorthing,maxNorthing);
    ...
    private static int scaleX(int easting,int minEasting,int maxEasting) {
    ...
    private static int scaleY(int northing,int minNorthing,int maxNorthing) {
    ...
    private static void drawLegend(Graphics2D g) {
        ...
        for (int i = 0; i < frequencies.length; i++) {
            ...
            g.setColor(getFrequencyColour(frequencies[i]));

            g.fillOval(x,currentY - 7,14,14);

            g.setColor(Color.BLACK);

            g.drawOval(x,currentY - 7,14,14);

            g.drawString(String.valueOf(frequencies[i]),x + 25,currentY + 5);
        }
        ...
        g.drawLine(x,edgeY,x + 20,edgeY);

        g.setColor(Color.BLACK);
        g.setFont(new Font("Arial", Font.PLAIN, 13));

        g.drawString("Interference connection",x + 30,edgeY + 5);
    }
}
